...
class GPUStats:
    ...
    def gpu_energy_j(self, elapsed_s: float) -> float:
    ...
    def end_epoch(self) -> dict:
        """Call after the last batch of an epoch. Stops the sampler thread and returns a stats dict."""
        self.stop_event.set()
        ...
        peak_pct = peak_gb / self.total_memory_gb * 100
        avg_util  = sum(self.epoch_samples) / len(self.epoch_samples)
        peak_util = max(self.epoch_samples)
    ...
    def summary(self) -> dict:
        ...
        peak_mem     = max(self.peak_mem_each)
        peak_mem_pct = peak_mem / self.total_memory_gb * 100
        return {
